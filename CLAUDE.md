...
**Не коммитить, пока не попросят явно.** Сделал изменения — оставь их в
рабочем дереве и скажи, что готово. Автор сам смотрит результат в браузере
(особенно всё, что касается внешнего вида) и решает, когда фиксировать.
Это же относится к `deploy`.
